...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	// }
		...
		// Above is the commented Clock Sweep Algorithm
	// Below is our implementation of LRU2
	for(int try_counter = 0; try_counter<10; try_counter++){
		uint32 ts1 = UINT32_MAX;
		uint32 ts2 = UINT32_MAX;
		BufferDesc *lru1_buf = NULL;
		uint32 lru1_state;
		
		BufferDesc *lru2_buf = NULL;
		uint32 lru2_state;
		
		// bool scanned_first = false;
		// bool scanned_second = false;
		...
		for(int i = 0; i<NBuffers; i++){
			...
			if(BUF_STATE_GET_REFCOUNT(local_buf_state) == 0){
				...
				if(ts1 > buf->timestamp){
					if(lru2_buf != NULL){
						UnlockBufHdr(lru2_buf, lru2_state);
					}
					ts2 = ts1;
					lru2_buf = lru1_buf;
					lru2_state = lru1_state;

					if( lru1_buf != NULL){
						UnlockBufHdr(lru1_buf, lru1_state);
					}
					ts1 = buf->timestamp;
					lru1_buf = buf;
					lru1_state = local_buf_state;
				}
				else if (ts2 > buf->timestamp){
					...
					ts2 = buf->timestamp;
					lru2_buf = buf;
					lru2_state = local_buf_state;
				}
				else{
					// Don't care about this buffer since was greater than ts1 and ts2. 
					UnlockBufHdr(buf, local_buf_state);
				}
			}
			else{
				// Didn't find a valid buffer
				UnlockBufHdr(buf, local_buf_state);
			}
